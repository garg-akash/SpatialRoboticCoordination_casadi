...
% clear;
clc;
Path = 'TextFiles/cross/';
A = importdata(strcat(Path,'path1.txt'));
B = importdata(strcat(Path,'constraints_traj.txt'));
...
c_A0 = importdata(strcat(Path,'constraints_A0.txt'));
c_A1 = importdata(strcat(Path,'constraints_A1.txt'));
c_b = importdata(strcat(Path,'constraints_b.txt'));
c_th = importdata(strcat(Path,'constraints_th.txt'));
if (reverse)
    c_A0 = flip(c_A0);
    c_A1 = flip(c_A1);
    c_b = flip(c_b);
...
for i = 1:length(Hfinal_2_ex)
    if (Hfinal_2_ex(i) > pi)
        cons_th(i,:) = cons_th(i,:) + 2*pi;
    elseif(Hfinal_2_ex(i) < -pi)
        cons_th(i,:) = cons_th(i,:) - 2*pi;
    end
end