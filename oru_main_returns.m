...
%     ind_left = 0;
    [V_opt, Omg_opt, P_opt, Th_opt, Px, Py, D_opt] = oru_sptl_dev(N, throw, del_t, robo_start, Xunit_1(ind_or), Yunit_1(ind_or), Hfinal_1(ind_or) ...
                                ...
                                , [Xunit_2(end);Yunit_2(end);Hfinal_2(end)], robo_omg(k), robo_vel(k), flag2);
...
%         + power((Py(N+1) - Yunit_2(end)),2)) - 3;
    ...
    vel_0(k+N+1-throw:k+N) = V_opt(N+1-throw:end);  %VVI for tracking reference
    omg_0(k+N+1-throw:k+N) = Omg_opt(N+1-throw:end);  %VVI for tracking reference
 
end
toc;
% fileID = fopen('35_path2_opt.txt','w');
% fprintf(fileID,'%f %f %f %f \n',[robo_traj;robo_th;robo_p]);
% fclose(fileID);
