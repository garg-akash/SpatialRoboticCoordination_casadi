function [v_out, omg_out, p_out, t_out, x_out, y_out, d_out] = oru_sptl_dev(N, throw, del_t, r_st, o1_px, o1_py, o1_h, r_px, r_py, r_h ...
                                ...
                                , A0, A1, b, th, vel_0, omg_0, goal, omg_pichla, vel_pichla, flag2, f_b, f_el)
import casadi.*
ind_ob_v = ind_ob;
ind_left_v = ind_left;
nx  = 4;
...
%     if(i==(N - throw) && ((t_sim - c_sim) <= (N - throw)))
    if(i==(N) &&(t_sim/2 < c_sim))
        disp('Entered!!!!!!!!!!!!!!')
        g = {g{:} res(1) res(2) res(3) res(4)};
        lbg = [lbg; goal(1); goal(2); goal(3); goal(4)];
        ubg = [ubg; goal(1); goal(2); goal(3); goal(4)];
    end
%     if(i==N)
%         disp('Entered!!!!!!!!!!!!!!')
%         r_px(i)
...
%     end
    J = J + T*(u(1)^2) + T*(u(2)^2);
%     J = J + T*((u(1) - vel_0(i))^2  + (u(2) - omg_0(i))^2);
    if(ismember(i,1:round(N/nv,0):N) && c_sim<=t_sim/2)
    if((flag1(i) == 1) && (ind_left>0))
        ...
        ind_left = ind_left - round(N/nv,0);
        ind_ob = ind_ob + round(N/nv,0);
